...
export default function AddReceiptModal({
    ...
    const modeOptions = useMemo(
        () => [
            ...
            { label: "UPI/NEFT", value: "upi" }, // ✅ send "upi" (keep consistent)
        ],
        []
    );
    ...
    const validate = () => {
        ...
        if (mode === "cheque") {
            ...
            // ifsc is nullable as per your api, so not required
        }
    ...
    const save = async () => {
        const err = validate();
        if (err) return onSave?.({ __error: err }); // (we handle toasts outside) OR alert(err)

        const payload = {
            type, // family / establishment

            family_id: type === "family" ? toStr(familyId) : null,
            establishment_no: type === "establishment" ? toStr(establishmentId) : null,
